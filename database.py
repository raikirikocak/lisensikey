...
def validate_key():
    ...
    # Memastikan JSON di file berisi key yang ada dalam request
    keys_data = load_keys()  # Load keys dari file atau database
    if input_key not in keys_data:
        return jsonify({"success": False, "message": "Key tidak valid"}), 404
    
    key_data = keys_data[input_key]  # Ambil data berdasarkan key
